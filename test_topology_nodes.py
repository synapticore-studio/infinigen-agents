...
def test_topology_analysis():
        ...
        try:
            ...
            if result["success"]:
                ...
                if topology_info:
                    ...
                    print(f"      - Manifold: {'✅ Yes' if topology_info.get('is_manifold', False) else '❌ No'}")
                    print(f"      - Has Boundary: {'✅ Yes' if topology_info.get('has_boundary', False) else '❌ No'}")
    ...
    if successful_tests:
        ...
        manifold_count = sum(1 for r in successful_tests if r.get("topology_info", {}).get("is_manifold", False))
        boundary_count = sum(1 for r in successful_tests if r.get("topology_info", {}).get("has_boundary", False))
        ...
        genus_values = [r.get("topology_info", {}).get("genus", 0) for r in successful_tests]
    ...
    try:
        ...
        for i, obj in enumerate(mesh_objects[:3]):  # Test first 3 objects
            ...
            if topology_info:
                ...
                print(f"      - Has Boundary: {topology_info.get('has_boundary', 'N/A')}")
                print(f"      - Genus: {topology_info.get('genus', 'N/A')}")
            else:
