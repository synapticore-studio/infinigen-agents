...
def test_sample_operations():
        ...
        try:
            ...
            if result["success"]:
                ...
                if terrain_obj:
                    ...
                    sample_modifiers = [mod for mod in modifiers if "TerrainDetail" in mod.name or "TerrainErosion" in mod.name]
                    
                    print(f"   🔧 Sample Operations Applied:")
                    print(f"      - Total modifiers: {len(modifiers)}")
                    print(f"      - Sample operation modifiers: {len(sample_modifiers)}")
                    ...
                    print(f"      - Detail level: {'High' if config.resolution >= 512 else 'Medium'}")
                ...
                print(f"❌ {config.terrain_type.value} terrain generation failed: {result.get('error', 'Unknown error')}")
                results.append({
                    "terrain_type": config.terrain_type.value,
                    "success": False,
    ...
    if successful_tests:
        ...
        avg_vertices = sum(r.get("vertex_count", 0) for r in successful_tests) / len(successful_tests)
        avg_faces = sum(r.get("face_count", 0) for r in successful_tests) / len(successful_tests)
        avg_modifiers = sum(r.get("sample_modifiers", 0) for r in successful_tests) / len(successful_tests)
        ...
        erosion_terrain_types = [r["terrain_type"] for r in successful_tests if r.get("sample_modifiers", 0) > 0]
...
def test_sample_node_creation():
    ...
    try:
        ...
        if node_group:
            ...
            sample_nodes = [node for node in node_group.nodes if "Sample" in node.bl_idname or "Raycast" in node.bl_idname]
        ...
        for i, obj in enumerate(mesh_objects[:2]):  # Test first 2 objects
            ...
            for detail_level in ["low", "medium", "high"]:
                ...
                if success:
                    ...
                    print(f"         ✅ Enhanced: {vertices_before} -> {vertices_after} vertices")
                else:
