...
from infinigen.agent_deps.core_deps import ValidationManagerDep
from infinigen.agent_deps.config_deps import InfinigenConfigDep
from infinigen.agent_tools.file_tools import FileManagerDep, LoggerDep

logger = logging.getLogger(__name__)


class ExportSpecialistAgent(BaseModel):
    """Agent specialized in data export and format conversion"""
    ...
    def export_scene_data(
        ...
        try:
            ...
            success = file_manager.save_json(export_metadata, export_file)
    ...
    def convert_mesh_format(
        ...
        try:
            ...
            success = file_manager.save_json(conversion_metadata, conversion_file)

            if success:
                logger_tool.info(f"Successfully converted mesh to {target_format}")
                return {
                    "success": True,
                    "conversion_settings": default_settings,
    ...
    def export_ground_truth_data(
        ...
        try:
            ...
            success = file_manager.save_json(gt_metadata, gt_file)

            if success:
                logger_tool.info(f"Successfully exported {len(gt_types)} GT types")
                return {
                    "success": True,
                    "export_settings": default_settings,
                    "gt_file": str(gt_file),
    ...
    def get_export_recommendations(
        self, target_application: str, data_type: str = "scene"
    ) -> Dict[str, Any]:
        ...
        try:
            recommendations = {
                ...
                "export_settings": self._get_export_settings_recommendations(
                    target_application
                ),
                "optimization_tips": self._get_export_optimization_tips(
                    target_application
                ),
            }
    ...
    def _get_format_recommendations(
        self, target_application: str, data_type: str
    ) -> List[str]:
    ...
    def _get_export_optimization_tips(self, target_application: str) -> List[str]:
